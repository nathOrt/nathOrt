$(document).ready(function () {
	...
	$('#contact-form').on('submit', function (event) {
		...
		var whatsappMessage = `Olá, meu nome é ${name}. Meu e-mail é ${email} e meu telefone é ${phone}. Mensagem: ${message}`;
		var whatsappUrl = `https://wa.me/5565992461257?text=${encodeURIComponent(whatsappMessage)}`;

		window.open(whatsappUrl, '_blank');

		$.magnificPopup.open({
			items: {
				src: '<div class="form-alert"><p>Sua mensagem foi enviada com sucesso!<br>Aguarde um retorno.</p></div>',
				type: 'inline'
			}
		});
	...
	$('body').on('click', '.hamburger-button', function (e) {
		e.preventDefault();
		var hamburgerMenuWrap = $('.menu-wrap');
		var hamburgerMenuItem = $('.menu-nav');
